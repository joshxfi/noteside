...
test.describe("keyboard chords", () => {
  ...
  test("Mod± zooms the editor font and Mod-Shift± the interface", async ({ page }) => {
    ...
    // Shift+Equal/Minus is now the RIGHT gesture: eventChord matches on
    // modifier FLAGS + key (folding a real layout's "+"/"_" back to =/-), so
    // both Playwright's synthesized Shift+= and real-hardware "+" land on
    // Mod-Shift-=. (The CM-era "press the glyph, not Shift+=" rule was an
    // artifact of CM's keyCode fallback and no longer applies.)
    await page.keyboard.press("ControlOrMeta+Shift+Equal");
    await expect.poll(() => cssVar("--ui-scale")).toBe("1.05");
    expect(await cssVar("--editor-size")).toBe("19px");
    await page.keyboard.press("ControlOrMeta+Shift+Minus");
